<template>
  <div>

    <v-toolbar
      rounded
      color="teal darken-4"
      title="انبار سوله اصلی"
    ></v-toolbar>
    <v-table class='rounded'>
      <thead>
        <!-- <tr>
          <th style="border-bottom:0.5px solid #000 ; border-left:0.5px solid #000" class="text-center">

          </th>          
        </tr> -->
        <tr>

          <th style="border-bottom:0.5px solid #000 ;border-left:0.5px solid #000" class="text-center text-h5" colspan="2">
            I
          </th>
          <th style="border-bottom:0.5px solid #000 ;border-left:0.5px solid #000" class="text-center text-h5" colspan="2">
            H 
          </th>           
          <th style="border-bottom:0.5px solid #000 ;border-left:0.5px solid #000" class="text-center text-h5" colspan="2">
            G 
          </th> 
          <th style="border-bottom:0.5px solid #000 ;border-left:0.5px solid #000" class="text-center text-h5" colspan="2">
            F 
          </th> 
          <th style="border-bottom:0.5px solid #000 ;border-left:0.5px solid #000" class="text-center text-h5" colspan="2">
            E 
          </th> 
          <th style="border-bottom:0.5px solid #000 ;border-left:0.5px solid #000" class="text-center text-h5" colspan="2">
            D 
          </th> 
          <th style="border-bottom:0.5px solid #000 ;border-left:0.5px solid #000" class="text-center text-h5" colspan="2">
            C 
          </th> 
          <th style="border-bottom:0.5px solid #000 ;border-left:0.5px solid #000" class="text-center text-h5" colspan="2">
            B 
          </th> 
          <th style="border-bottom:0.5px solid #000 ;border-left:0.5px solid #000" class="text-center text-h5" colspan="2">
            A
          </th>
          <th style="border-bottom:0.5px solid #000" class="text-center" colspan="2">
            *
          </th>
        </tr>

        <tr>
          <!-- I -->
          <th style="border-bottom:0.5px solid #000 ;border-left:0.5px solid #000 ; border-right:1.5px solid #000" class="text-center">
            تعداد
          </th>
          <th style="border-bottom:0.5px solid #000 ;border-left:0.5px solid #000" class="text-center">
            مدل
          </th>
          <!-- /I -->

          <!-- H -->
          <th style="border-bottom:0.5px solid #000 ;border-left:0.5px solid #000" class="text-center">
            تعداد
          </th>
          <th style="border-bottom:0.5px solid #000 ;border-left:0.5px solid #000" class="text-center">
            مدل
          </th>
          <!-- /H -->

          <!-- G -->
          <th style="border-bottom:0.5px solid #000 ;border-left:0.5px solid #000" class="text-center">
            تعداد
          </th>
          <th style="border-bottom:0.5px solid #000 ;border-left:0.5px solid #000" class="text-center">
            مدل
          </th>
          <!-- /G -->

          <!-- F -->
          <th style="border-bottom:0.5px solid #000 ;border-left:0.5px solid #000" class="text-center">
            تعداد
          </th>
          <th style="border-bottom:0.5px solid #000 ;border-left:0.5px solid #000" class="text-center">
            مدل
          </th>
          <!-- /F -->

          <!-- E -->
          <th style="border-bottom:0.5px solid #000 ;border-left:0.5px solid #000" class="text-center">
            تعداد
          </th>
          <th style="border-bottom:0.5px solid #000 ;border-left:0.5px solid #000" class="text-center">
            مدل
          </th>
          <!-- /E -->

          <!-- D -->
          <th style="border-bottom:0.5px solid #000 ;border-left:0.5px solid #000" class="text-center">
            تعداد
          </th>
          <th style="border-bottom:0.5px solid #000 ;border-left:0.5px solid #000" class="text-center">
            مدل
          </th>
          <!-- /D -->

          <!-- C -->
          <th style="border-bottom:0.5px solid #000 ;border-left:0.5px solid #000" class="text-center">
            تعداد
          </th>
          <th style="border-bottom:0.5px solid #000 ;border-left:0.5px solid #000" class="text-center">
            مدل
          </th>
          <!-- /C -->

          <!-- B -->
          <th style="border-bottom:0.5px solid #000 ;border-left:0.5px solid #000" class="text-center">
            تعداد
          </th>
          <th style="border-bottom:0.5px solid #000 ;border-left:0.5px solid #000" class="text-center">
            مدل
          </th>
          <!-- /B -->

          <!-- A -->
          <th style="border-bottom:0.5px solid #000 ;border-left:0.5px solid #000" class="text-center">
            تعداد
          </th>
          <th style="border-bottom:0.5px solid #000 ;border-left:0.5px solid #000" class="text-center">
            مدل
          </th>
          <!-- /A -->

          <!-- J -->
          <th style="border-bottom:0.5px solid #000 ;border-left:0.5px solid #000" class="text-center">
            *
          </th>
          <th style="border-bottom:0.5px solid #000 ;border-left:0.5px solid #000" class="text-center">
            **
          </th>
          <!-- /J -->
        </tr>
      </thead>

      <tbody>
        <tr
          v-for="(item,index) in cells"
          :key="index"
        >
          
          <!-- I -->
          <td class="smallPaddingCell" style="border-bottom:0.5px solid #000 ;border-left:0.5px solid #000 ; border-right:1.5px solid #000">
            <v-btn size="large" color="warning" variant="tonal" class="iconBtnSmall">         
              {{ item.I.qty }}
            </v-btn>            
          </td>
          <td class="smallPaddingCell" style="border-bottom:0.5px solid #000 ;border-left:5px solid #000">
            <editTable v-if="item.I.qty != null && item.I.model != '' " :itemSelected="item.I" :item="item"  />
            <addInCell v-if="item.I.qty == null && item.I.model == '' " :itemSelected="item.I" :item="item"  />
          </td>
          <!-- I -->

          <!-- H -->
          <td  class="smallPaddingCell" style="border-bottom:0.5px solid #000 ;border-left:0.5px solid #000 ; border-right:5px solid #000">
            <v-btn  size="large" color="warning" variant="tonal" class="iconBtnSmall">         
              {{ item.H.qty }}
            </v-btn>            
          </td>
          <td class="smallPaddingCell" style="border-bottom:0.5px solid #000 ;border-left:0.5px solid #000">
            <editTable v-if="item.H.qty != null && item.H.model != '' " :itemSelected="item.H" :item="item"  />
            <addInCell v-if="item.H.qty == null && item.H.model == '' " :itemSelected="item.H" :item="item"  />
          </td>
          <!-- H -->

          <!-- G -->
          <td  class="smallPaddingCell" style="border-bottom:0.5px solid #000 ;border-left:0.5px solid #000">
            <v-btn  size="large" color="warning" variant="tonal" class="iconBtnSmall">         
              {{ item.G.qty }}
            </v-btn>            
          </td>
          <td class="smallPaddingCell" style="border-bottom:0.5px solid #000 ;border-left:5px solid #000">
            <editTable v-if="item.G.qty != null && item.G.model != '' " :itemSelected="item.G" :item="item"  />
            <addInCell v-if="item.G.qty == null && item.G.model == '' " :itemSelected="item.G" :item="item"  />
          </td>
          <!-- G -->

          <!-- F -->
          <td  class="smallPaddingCell" style="border-bottom:0.5px solid #000 ;border-left:0.5px solid #000 ; border-right:5px solid #000">
            <v-btn  size="large" color="warning" variant="tonal" class="iconBtnSmall">         
              {{ item.F.qty }}
            </v-btn>            
          </td>
          <td class="smallPaddingCell" style="border-bottom:0.5px solid #000 ;border-left:0.5px solid #000">
            <editTable v-if="item.F.qty != null && item.F.model != '' " :itemSelected="item.F" :item="item"  />
            <addInCell v-if="item.F.qty == null && item.F.model == '' " :itemSelected="item.F" :item="item"  />
          </td>
          <!-- F -->

          <!-- E -->
          <td  class="smallPaddingCell" style="border-bottom:0.5px solid #000 ;border-left:0.5px solid #000">
            <v-btn  size="large" color="warning" variant="tonal" class="iconBtnSmall">         
              {{ item.E.qty }}
            </v-btn>            
          </td>
          <td class="smallPaddingCell" style="border-bottom:0.5px solid #000 ;border-left:5px solid #000">
            <editTable v-if="item.E.qty != null && item.E.model != '' " :itemSelected="item.E" :item="item"  />
            <addInCell v-if="item.E.qty == null && item.E.model == '' " :itemSelected="item.E" :item="item"  />
          </td>
          <!-- E -->

          <!-- D -->
          <td  class="smallPaddingCell" style="border-bottom:0.5px solid #000 ;border-left:0.5px solid #000 ; border-right:5px solid #000">
            <v-btn  size="large" color="warning" variant="tonal" class="iconBtnSmall">         
              {{ item.D.qty }}
            </v-btn>            
          </td>
          <td class="smallPaddingCell" style="border-bottom:0.5px solid #000 ;border-left:0.5px solid #000">
            <editTable v-if="item.D.qty != null && item.D.model != '' " :itemSelected="item.D" :item="item"  />
            <addInCell v-if="item.D.qty == null && item.D.model == '' " :itemSelected="item.D" :item="item"  />
          </td>
          <!-- D -->

          <!-- C -->
          <td  class="smallPaddingCell" style="border-bottom:0.5px solid #000 ;border-left:0.5px solid #000">
            <v-btn  size="large" color="warning" variant="tonal" class="iconBtnSmall">         
              {{ item.C.qty }}
            </v-btn>            
          </td>
          <td class="smallPaddingCell" style="border-bottom:0.5px solid #000 ;border-left:5px solid #000">
            <editTable v-if="item.C.qty != null && item.C.model != '' " :itemSelected="item.C" :item="item"  />
            <addInCell v-if="item.C.qty == null && item.C.model == '' " :itemSelected="item.C" :item="item"  />
          </td>
          <!-- C -->

          <!-- B -->
          <td  class="smallPaddingCell" style="border-bottom:0.5px solid #000 ;border-left:0.5px solid #000 ; border-right:5px solid #000">
            <v-btn  size="large" color="warning" variant="tonal" class="iconBtnSmall">         
              {{ item.B.qty }}
            </v-btn>            
          </td>
          <td class="smallPaddingCell" style="border-bottom:0.5px solid #000 ;border-left:0.5px solid #000">
            <editTable v-if="item.B.qty != null && item.B.model != '' " :itemSelected="item.B" :item="item"  />
            <addInCell v-if="item.B.qty == null && item.B.model == '' " :itemSelected="item.B" :item="item"  />
          </td>
          <!-- B -->

          <!-- A -->
          <td  class="smallPaddingCell" style="border-bottom:0.5px solid #000 ;border-left:0.5px solid #000">
            <v-btn  size="large" color="warning" variant="tonal" class="iconBtnSmall">         
              {{ item.A.qty }}
            </v-btn>            
          </td>
          <td class="smallPaddingCell" style="border-bottom:0.5px solid #000 ;border-left:5px solid #000">
            <editTable v-if="item.A.qty != null && item.A.model != '' " :itemSelected="item.A" :item="item"  />
            <addInCell v-if="item.A.qty == null && item.A.model == '' " :itemSelected="item.A" :item="item"  />
          </td>
          <!-- A -->


          <td v-if="(index % 2) == 0" style="border-bottom:0.5px solid #000 ;border-left:0.5px solid #000" rowspan="2">
          <!-- <div v-if="index < 10"> {{ (((index/2)+1)) }} </div>
          <div v-if="index >= 10"> {{ (((index/2)%5)+1) }} </div> -->
           {{ item.subGap }}
          </td>
          
          <td v-if="(index % 10) == 0" style="border-bottom:0.5px solid #000 ; border-left:1.5px solid #000" rowspan="10">{{ item.gap }}
          </td>
      
        </tr>

      </tbody>
    </v-table>
    <!-- <button @click="piiiii">pipipi</button> -->
  </div>
</template>

<script>
  import editTable from '../../editCell.vue'
  import addInCell from '../../addInCell.vue'
  import { reactive } from '@vue/reactivity'
  import {useOrdibeheshtStorageStore} from '../../store/ordibeheshtStorageStore'

  export default {
    components:{
      editTable,
      addInCell,
    },

    data () {
      return {
        cells: [
          {
            location : 'سوله اصلی',
            gap : 8,
            subGap: 5,
            line: 'اول',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          { 
            location : 'سوله اصلی',
            gap : 8,
            subGap: 5,
            line: 'دوم',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty:23,
              model:'pw20'
            },
          },
          {
            location : 'سوله اصلی',
            gap : 8,
            subGap: 4,
            line: 'اول',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 8,
            subGap: 4,
            line: 'دوم',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 8,
            subGap: 3,
            line: 'اول',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          { 
            location : 'سوله اصلی',
            gap : 8, 
            subGap: 3,
            line: 'دوم',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 8,
            subGap: 2,
            line: 'اول',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 8,
            subGap: 2,
            line: 'دوم',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 8,
            subGap: 1,
            line: 'اول',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: '----',
              model:'کمپرسور'
            },
          },
          {
            location : 'سوله اصلی',
            gap : 8,
            subGap: 1,
            line: 'دوم',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: '----',
              model:'کمپرسور'
            },
          },
          {
            location : 'سوله اصلی',
            gap : 7,
            subGap: 5,
            line: 'اول',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 7,
            subGap: 5,
            line: 'دوم',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 7,
            subGap: 4,
            line: 'اول',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 7,
            subGap: 4,
            line: 'دوم',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 7,
            subGap: 3,
            line: 'اول',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 7,
            subGap: 3,
            line: 'دوم',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 7,
            subGap: 2,
            line: 'اول',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 7,
            subGap: 2,
            line: 'دوم',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 7,
            subGap: 1,
            line: 'اول',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 7,
            subGap: 1,
            line: 'دوم',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 6,
            subGap: 5,
            line: 'اول',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 6,
            subGap: 5,
            line: 'دوم',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 6,
            subGap: 4,
            line: 'اول',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 6,
            subGap: 4,
            line: 'دوم',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 6,
            subGap: 3,
            line: 'اول',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 6,
            subGap: 3,
            line: 'دوم',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 6,
            subGap: 2,
            line: 'اول',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 6,
            subGap: 2,
            line: 'دوم',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 6,
            subGap: 1,
            line: 'اول',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 6,
            subGap: 1,
            line: 'دوم',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 5,
            subGap: 5,
            line: 'اول',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 5,
            subGap: 5,
            line: 'دوم',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 5,
            subGap: 4,
            line: 'اول',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 5,
            subGap: 4,
            line: 'دوم',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 5,
            subGap: 3,
            line: 'اول',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 5,
            subGap: 3,
            line: 'دوم',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 5,
            subGap: 2,
            line: 'اول',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 5,
            subGap: 2,
            line: 'دوم',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 5,
            subGap: 1,
            line: 'اول',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 5,
            subGap: 1,
            line: 'دوم',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 4,
            subGap: 5,
            line: 'اول',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 4,
            subGap: 5,
            line: 'دوم',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 4,
            subGap: 4,
            line: 'اول',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 4,
            subGap: 4,
            line: 'دوم',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 4,
            subGap: 3,
            line: 'اول',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 4,
            subGap: 3,
            line: 'دوم',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 4,
            subGap: 2,
            line: 'اول',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 4,
            subGap: 2,
            line: 'دوم',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 4,
            subGap: 1,
            line: 'اول',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 4,
            subGap: 1,
            line: 'دوم',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 3,
            subGap: 5,
            line: 'اول',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 3,
            subGap: 5,
            line: 'دوم',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 3,
            subGap: 4,
            line: 'اول',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 3,
            subGap: 4,
            line: 'دوم',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 3,
            subGap: 3,
            line: 'اول',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 3,
            subGap: 3,
            line: 'دوم',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 3,
            subGap: 2,
            line: 'اول',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 3,
            subGap: 2,
            line: 'دوم',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 3,
            subGap: 1,
            line: 'اول',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 3,
            subGap: 1,
            line: 'دوم',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 2,
            subGap: 5,
            line: 'اول',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 2,
            subGap: 5,
            line: 'دوم',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 2,
            subGap: 4,
            line: 'اول',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 2,
            subGap: 4,
            line: 'دوم',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 2,
            subGap: 3,
            line: 'اول',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 2,
            subGap: 3,
            line: 'دوم',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 2,
            subGap: 2,
            line: 'اول',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 2,
            subGap: 2,
            line: 'دوم',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 2,
            subGap: 1,
            line: 'اول',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 2,
            subGap: 1,
            line: 'دوم',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 1,
            subGap: 5,
            line: 'اول',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 1,
            subGap: 5,
            line: 'دوم',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 1,
            subGap: 4,
            line: 'اول',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 1,
            subGap: 4,
            line: 'دوم',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 1,
            subGap: 3,
            line: 'اول',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 1,
            subGap: 3,
            line: 'دوم',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 1,
            subGap: 2,
            line: 'اول',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 1,
            subGap: 2,
            line: 'دوم',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 1,
            subGap: 1,
            line: 'اول',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
          {
            location : 'سوله اصلی',
            gap : 1,
            subGap: 1,
            line: 'دوم',
            A:{
              column: 'A',
              qty: null,
              model:''
            },
            B:{
              column: 'B',
              qty: null,
              model:''
            },
            C:{
              column: 'C',
              qty: null,
              model:''
            },
            D:{
              column: 'D',
              qty: null,
              model:''
            },
            E:{
              column: 'E',
              qty: null,
              model:''
            },
            F:{
              column: 'F',
              qty: null,
              model:''
            },
            G:{
              column: 'G',
              qty: null,
              model:''
            },
            H:{
              column: 'H',
              qty: null,
              model:''
            },
            I:{
              column: 'I',
              qty: null,
              model:''
            },
          },
        ],
      }

    },
    setup(props){

      const colorStatus = reactive(props.colorTheme)
      const store = useOrdibeheshtStorageStore()

      // const cells = computed(() => store.getMainStorageCells);

      // function piiiii(){
      //   for(const cellol in cells._value){
      //     console.log(cellol)
      //   }
      // }

      const loading = reactive({
        getStoreCells: false,
      })

      async function getStoreCells(){
        loading.getStoreCells = true;
        await store.getStoreMainStorageCells();
        loading.getStoreCells = false;
      }

      return{
        colorStatus,
        getStoreCells,
        loading,
      }
    }
  }
</script>

<style>

  .iconBtnSmall{
    --v-btn-height: 30px;
    min-height: -webkit-fill-available;
    min-width: -webkit-fill-available!important;
    border-radius: 0;
  }

  .smallPaddingCell{
    padding: 0rem!important;
    margin: 0rem!important;
  }

  .borderForSubGap{
    border-bottom:5px solid #000
  }

  /* .borderForSubGap{
    border-bottom:3px solid #000
  } */

</style>